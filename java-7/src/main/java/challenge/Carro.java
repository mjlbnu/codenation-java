...
public class Carro {
  ...
  private void validaCarro(Motorista motorista, String placa, Cor cor) {
    if (placa == null || cor == null) {
      throw new NullPointerException("Placa ou cor não informados.");
    }
    if (motorista == null) {
      throw new EstacionamentoException("Motorista não informado.");
    }
    if (motorista.getIdade() < 18) {
      throw new EstacionamentoException("Motorista menor de idade.");
    }
    if (motorista.getPontos() > 20) {
      throw new EstacionamentoException("A habilitação do motorista está suspensa.");
    }
  }
}
